...
def init_model(m_path, device, vocab):
    ...
    lm_args = ckpt['args']
    lm_vocab = Vocab(vocab, min_occur_cnt=1, specials=[])
...
def combine_inference(previous_sentence, retrieval_sentence):
    # 输入： 上一句话 和 检索到的句子
    ...
    query = previous_sentence + retrieval_sentence
    ...
    r1 = greedy(lm_model, lm_vocab, device, s, max_len)[pre_len:]

    r2 = beam_search(lm_model, lm_vocab, device, s, max_len)[pre_len:]

    r3 = top_k_inc(lm_model, lm_vocab, device, s, 5, max_len)[pre_len:]

    r4 = top_k_inc(lm_model, lm_vocab, device, s, 10, max_len)[pre_len:]

    r5 = top_k_inc(lm_model, lm_vocab, device, s, 20, max_len)[pre_len:]

    r6 = top_k_inc(lm_model, lm_vocab, device, s, 50, max_len)[pre_len:]

    r7 = top_k_inc(lm_model, lm_vocab, device, s, 500, max_len)[pre_len:]

    r8 = top_p_inc(lm_model, lm_vocab, device, s, 20, 0.95, max_len)[pre_len:]
    
    print("previous sentence: ", previous_sentence)
    print("retrieval sentence: ", retrieval_sentence)
    print("greedy: ", r1)
    print("bm5: ", query+r2)
    print("tk5: ", r3)
    print("tk10: ", r4)
...
if __name__ == '__main__':
    pre = '宇宙文明世界最高的标准之一'
    retri = '不允许高级文明向低级文明传递信息'
    combine_inference(pre, retri)